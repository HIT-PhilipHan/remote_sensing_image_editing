...
To validate the proposed image editing method for supporting remote sensing tasks, we used the disaster assessment dataset, generating post-disaster images with the prompt ``a picture of ground after tsunami" to construct the training dataset,which is shown as follows:
